...
- **未完了(ユーザーの残タスク)**:
  1. ~~Cloudflareで上記DNSレコード(`A`レコード2件)を追加する~~ → 完了(プロキシをDNSのみに直す作業も含めて完了)
  2. (任意・後回し可)VercelダッシュボードでGitHubリポジトリとの連携を承認する(`vercel git connect`、これができると`git push`だけで自動デプロイされる。今は`vercel --prod`で手動デプロイしている)

### 13-1. 公開後に発見・修正した性能問題(2026-09-05)

DNS切り替え後、「ページ移動が遅い・読み込めない」という報告があり、調査したところ2つの問題が重なっていた。

1. **Vercelの実行リージョンがSupabase(東京)と異なっていた**: デフォルトでは`iad1`(米国東部)でサーバーレス関数が実行されており、DBへの全クエリが太平洋を往復していた(トップページで4〜8秒)。`vercel.json`に`"regions": ["hnd1"]`(東京)を追加して解消(トップページ0.2〜1秒に改善)。
2. **`connection_limit=1`が並行クエリを直列化していた**: 前述の接続数エラー対策で`connection_limit=1`にしていたが、これによりPromise.allで並行実行していたはずのクエリが1本の接続で順番待ちになり、遅延が増えていた。Transaction mode poolerは多くのクライアント接続を少数のバックエンド接続に多重化する仕組みのため、`connection_limit=5`に上げても接続数エラーは再発しないことを確認し、この値に変更した。

この2つを直した上で、30件の同時アクセスでもエラーなく、0.2〜1.3秒で応答することを確認済み。
